...
# Check if UFW is active and open port 2323 if needed
if sudo ufw status | grep -q "Status: active"; then
    sudo ufw allow 2323/tcp
fi

# Check if iptables is active and open port 1080 if needed
if sudo iptables -L | grep -q "ACCEPT     tcp  --  anywhere             anywhere             tcp dpt:2323"; then
    echo "Port 2323 is already open in iptables."
else
